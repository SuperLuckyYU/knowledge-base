<template>
  <a-modal title="选择知识条目" :visible="true" :maskClosable="false" :closable="false" centered width="80%" destroyOnClose
    :body-style="{ 'height': '80vh', 'overflow-y': 'auto' }" @ok="handleSubmit" @cancel="onModalClose">
    <a-card class="search-box">
      <a-form ref="searchFormRef" layout="inline" :model="formState">
        <a-row :gutter="[24, 16]">
          <a-col>
            <a-form-item label="标题" name="name">
              <a-input v-model:value.trim="formState.name" placeholder="请输入知识条目标题" autocomplete="off" allow-clear />
            </a-form-item>
          </a-col>
          <a-col>
            <a-form-item>
    ...
    <select-knowledge-table rowKey="id" :data-source="dataSource" :pagination="pagination" bordered @change="onTableChange">
...
const handleChange = (row: Record<string, any>) => {
  ...
  if (row.checked) {
    ...
    state.selectedRow.splice(state.selectedRow.findIndex(item => item.id == row.id), 1)
  }
};
